...
/**
 * Extrait la clé API depuis la requête (en-tête Authorization ou body)
 */
function extractApiKey(request: NextRequest, formData: FormData): string | null {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extraire la clé API
    const apiKey = extractApiKey(request, formData)
    
    if (!apiKey) {
      return NextResponse.json(
        { error: 'Clé API requise. Fournissez-la dans l\'en-tête Authorization: Bearer <apiKey> ou dans le body comme apiKey' },
        { status: 401 }
      )
    }

    // Valider la clé API et récupérer l'utilisateur
    ...
    const userName = user.name || user.identifier || 'user'
    ...
    // Créer un log d'activité (avec indication que c'est via API)
    ...
    console.error('Erreur lors de l\'upload du fichier JSON via API:', error)
    return NextResponse.json(
